...
Evolve::Unit::Unit(t_brain settings, float MEANRADIUS, float REPRATE,
  ...
  age      = 0;
  MUTRATE1 = MUTARATE1 +
             MEANRADIUS * randf(-conf::METAMUTRATE1, conf::METAMUTRATE1) *
             3; //chance of mutations. MEANRADIUS turns off the throw for children
  MUTRATE2 = MUTARATE2 +
             MEANRADIUS * randf(-conf::METAMUTRATE2, conf::METAMUTRATE2) *
             15; //size of mutations
  radius   = randf(MEANRADIUS * 0.2, MEANRADIUS * 2.2);

  //triggers, counters, and layer interaction
  near                   = false;
  freshkill              = 0;
  species                = randi(-conf::NUMBOTS * 200, conf::NUMBOTS *
                                                       200); //related to numbots because it's a good relationship
  gencount               = 0;
  repcounter             = REPRATE;
  numbabies              = randi(1, 6);
...
Evolve::Unit Evolve::Unit::reproduce(Evolve::Unit that, float REPRATE) {
  float MR  = randf(std::min(this->MUTRATE1, that.MUTRATE1),
                    std::max(this->MUTRATE1, that.MUTRATE1));
  float MR2 = randf(std::min(this->MUTRATE2, that.MUTRATE2),
                    std::max(this->MUTRATE2, that.MUTRATE2));
  ...
  a2.stomach[Stomach::PLANT] = randf(0, 1) < 0.5 ? this->stomach[Stomach::PLANT]
                                                 : that.stomach[Stomach::PLANT];
  a2.stomach[Stomach::MEAT]  = randf(0, 1) < 0.5 ? this->stomach[Stomach::MEAT]
                                                 : that.stomach[Stomach::MEAT];
  a2.stomach[Stomach::FRUIT] = randf(0, 1) < 0.5 ? this->stomach[Stomach::FRUIT]
                                                 : that.stomach[Stomach::FRUIT];
  ...
  a2.eye_see_unit_mod =
          randf(0, 1) < 0.5 ? this->eye_see_unit_mod : that.eye_see_unit_mod;
//	a2.eye_see_cell_mod= randf(0,1)<0.5 ? this->eye_see_cell_mod : that.eye_see_cell_mod;
  ...
  a2.temperature_preference = randf(0, 1) < 0.5 ? this->temperature_preference
                                                : that.temperature_preference;
  ...
  if (randf(0, 1) < MR * 5)
    a2.stomach[Stomach::FRUIT] = cap(
            randn(a2.stomach[Stomach::FRUIT], MR2 * 10));
  ...
  if (randf(0, 1) < MR)
    a2.MUTRATE1 = abs(randn(a2.MUTRATE1, conf::METAMUTRATE1));
  if (randf(0, 1) < MR)
    a2.MUTRATE2 = abs(randn(a2.MUTRATE2, conf::METAMUTRATE2));
...
//	if (randf(0,1)<MR) a2.eye_see_cell_mod= randn(a2.eye_see_cell_mod, MR2);
  ...
  if (randf(0, 1) < MR * 2)
    a2.temperature_preference = cap(randn(a2.temperature_preference, MR2 * 2));
  if (randf(0, 1) < MR * 2)
    a2.lungs                  = cap(randn(a2.lungs, MR2 * 2));
