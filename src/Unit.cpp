...
Evolve::Unit::Unit(t_brain settings, float MEANRADIUS, float REPRATE, float MUTARATE1, float MUTARATE2) :
        brain(settings)
{
  //basics
  id= 0;
...
Evolve::Unit Evolve::Unit::reproduce(Evolve::Unit that, float REPRATE)
{
  ...
  //NOTES: Evolve::Unit "this" is mother, Evolve::Unit "that" is father, Evolve::Unit "a2" is daughter
  //if a single parent's trait is required, use the mother's (this->)
  Evolve::Unit a2(this->brain.getBrain(), 0, REPRATE, MR, MR2);
...
"Killed by Unknown Factor!			Evolve::Unit.cpp	~ln 87  Note: if no other message is applied, this will pop up
